...
const App: React.FC = () => {
  ...
  const AppHeader = () => (
    <header className="absolute top-0 left-0 right-0 z-20 flex flex-col items-center p-4 md:p-6">
        ...
        <div className="flex justify-center w-full mt-2 gap-4">
            ...
            <button className="flex items-center justify-center w-10 h-10 text-white transition-colors duration-200 bg-black rounded-full bg-opacity-20 backdrop-blur-sm hover:bg-opacity-30" onClick={() => setShowIconInfo(true)}>
                <ResourcesIcon />
            </button>
            {/* Second ResourcesIcon, also pressable, shows image info */}
            <button className="flex items-center justify-center w-10 h-10 text-white transition-colors duration-200 bg-black rounded-full bg-opacity-20 backdrop-blur-sm hover:bg-opacity-30" onClick={() => setShowImageInfo(true)}>
                <ResourcesIcon />
            </button>
        </div>
        {/* Popup for icon info */}
  ...
  useEffect(() => {
    // Check if any message contains the word "hospital" or "wait time"
    const hospitalMentioned = messages.some(
      msg => typeof msg.text === "string" && (
        msg.text.toLowerCase().includes("hospital") ||
        msg.text.toLowerCase().includes("wait time")
      )
    );
    if (hospitalMentioned && !showNearestHospital) {
      saveHospitalWaitTimesSnapshot();
      requestUserLocation();
    }
  }, [messages, saveHospitalWaitTimesSnapshot, showNearestHospital]);
