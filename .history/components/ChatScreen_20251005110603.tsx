...
const ChatScreen: React.FC<ChatScreenProps> = ({
  ...
  const [isMobileMenu, setIsMobileMenu] = useState(window.innerWidth < 1360);

  useEffect(() => {
    const handleResize = () => setIsMobileMenu(window.innerWidth < 1360);
        ...
        {isMobileMenu && (
          <div className="mb-3">
            <button
              ...
              className="px-4 py-2 text-sm font-medium rounded bg-sky-900 text-sky-200 hover:bg-sky-800"
...
${VOICE_PROMPT}
      ...
      {showHospitalModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="p-8 bg-slate-800 rounded-lg max-w-lg w-full relative">
            <button
              onClick={() => setShowHospitalModal(false)}
              className="absolute top-4 right-4 px-3 py-1 bg-sky-900 text-sky-200 rounded hover:bg-sky-800"
            >
              Close
            </button>
            <h2 className="mb-4 text-lg font-bold text-sky-300">Hospital Wait Times</h2>
            {/* Example hospital info, replace with real data if needed */}
            <div className="space-y-3">
              <div className="font-semibold">Nearest Hospital: Example Hospital</div>
              <div>Wait Time: 45 min</div>
        ...
        {streamingOutput && (
            <div className="flex items-start gap-3 justify-start">
                ...
                {(isThinking || isWriting) && (
                    <div className="flex items-start gap-3 justify-start">
                        <img src="/leave_animation.gif" alt="Writing..." style={{ width: 40, height: 40, objectFit: 'contain', marginRight: 4, borderRadius: 8 }} />
                        <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-sky-900 rounded-full text-sky-400">
                                <BotIcon />
                        </div>
